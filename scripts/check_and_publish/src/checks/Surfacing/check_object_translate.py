...
top_hierarchy = get_top_outline_name()
maya_node = MayaNode(top_hierarchy)
# 定义需要检测的层级路径
hierarchy = []
for hie in maya_node.all_descendents:
    if (hie.node_type == "transform" and hie.get_children()) or (
            hie.node_type == "transform" and not hie.get_children()):
        hierarchy.append(hie.path)
hierarchy.append(top_hierarchy)
...
def start_check():
    ...
    for node in hierarchy:
        if cmds.objExists(node):
            ...
            if cmds.objExists(node) and cmds.objectType(node) == "transform":  # 确保是有效的transform节点
                child_pivot = cmds.xform(node, q=True, rp=True, ws=True)  # 获取物体枢轴位置
