...
importlib.reload(ensure_name_ui)


class EnsureNameView(QDialog):
    def __init__(self, format_list: list, parent=None):
    ...
    def build(self):
        if self.maya_data.pipeline == "Surfacing":
            cgt_asset = CGTAssetTask(self.maya_data.project_database, self.maya_data.task_id)
    ...
    def change_format(self, name: str):
        self.ui.lineEdit_name.setEnabled(True)
        if "*" not in name:
