...
class MayaShaderPy2:
    ...
    def export_material_info(self, yaml_file, render_type):
        ...
        self.cmds.select(cl=1)
        objects = self.cmds.ls(type="mesh", l=True)
        for obj in objects:
            obj_material = self.cmds.listConnections(obj, type="shadingEngine")
            if not obj_material:
                continue
            for shading_connection in self.cmds.listConnections(obj_material):
                mat = self.cmds.ls(shading_connection, mat=1)
                if not mat:
                    continue
                if mat == ["lambert1"]:
                    continue
                ...
                for attribute in need_attributes:
                    attribute_dict[attribute] = self.cmds.getAttr("{}.{}".format(obj, attribute))
    ...
    def export_shader(self, ma_file):
        ...
        if not shader_list:
            return
        self.cmds.select(shader_list, allDagObjects=False, noExpand=True)
        self.mel.eval(
    ...
    def assign_shader(self, mesh_node, shader_node):
        ...
        self.cmds.hyperShade(assign=shader_node)

    def get_material(self, material_type):
        return tuple(MayaAttributePy2(i) for i in self.cmds.ls(type=material_type.value))
