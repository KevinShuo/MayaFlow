...
class MayaNodePy2(object):
    ...
    def get_node_type(self, node_type):
        return self.cmds.ls(type=node_type)
    ...
    @property
    def top_level_transform(self):
        """
         获取 mesh 节点的最顶层 group 节点。
         group 是指最上层的 transform 或 group 节点。
         """
        parent = self.cmds.listRelatives(self.node_path, parent=True)
        # 遍历父节点，直到找到最顶层的 group 节点
        while parent:
            # 判断父节点是否是 transform 类型（可能是 group）
            parent_type = self.cmds.nodeType(parent[0])
            if parent_type == 'transform':  # 如果是 transform（包括 group）
                mesh_node = parent[0]
                parent = self.cmds.listRelatives(mesh_node, parent=True)
            else:
                break  # 遇到非 transform 类型的节点，停止查找
        return mesh_node
