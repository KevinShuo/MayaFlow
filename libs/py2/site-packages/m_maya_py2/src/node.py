...
import enum
from m_maya_py2.src.attribute import MayaAttribute
...
class MayaNode(object):
    ...
    def create_node(name, node_type):
        # type: (str,MayaNodeType) -> 'MayaNode'
        import maya.cmds as cmds
        node = cmds.createNode(node_type.name, n=name)
        return MayaNode(node)
    ...
    def iter_parents(self):
        ...
        for parent in parents:
            yield MayaNode(parent)

    @property
    def parents(self):
        return tuple([MayaNode(i) for i in self.cmds.listRelatives(self.node_path, parent=True, f=True)])
