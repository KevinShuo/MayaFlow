...
class MayaGroupPy2(MayaNodePy2):
    def __init__(self, node_path):
        # type: (Optional[AnyStr]) -> None
        super(MayaGroupPy2, self).__init__(node_path)
        self.node_path = node_path

    def create_new(self, name, return_group=False):
        # type: (str,bool) -> Union['MayaNodePy2','MayaGroupPy2']
        group_name = self.cmds.group(n=name, empty=True)
        return MayaNodePy2(group_name) if not return_group else MayaGroupPy2(group_name)
    ...
    def ensure_node_creation(self):
        ...
        ### **依赖**
        - 使用 `MayaNodePy2` 来检查节点是否存在。
        - 使用 `MayaGroupPy2` 来创建新的组节点并添加子节点。
    ...
    def add_child(self, name, return_group=False):
        ...
        return MayaNodePy2(group) if not return_group else MayaGroupPy2(group)

    def add_children(self, names, return_group=False):
        # type: (str,bool) -> Generator[Union['MayaNodePy2','MayaGroupPy2']]
        for n in names:
