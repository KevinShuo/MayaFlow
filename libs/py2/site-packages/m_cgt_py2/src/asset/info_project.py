# -*- coding: utf-8 -*-
from m_cgt_py2.src.login import PermissionStrategy, AdminUserStrategy
...
class CGTAssetInfo:
    ...
    def get_data(self, filter_param=None):
        if filter_param is None:
            filter_param = []
        task_id = self.t_tw.info.get_id(db=self.project_db, module=self.module, filter_list=filter_param, limit="20000")
        data = self.t_tw.info.get(self.project_db, self.module, task_id, self.fields, limit="20000")
        if data is None:
            return
        for item in data:
            yield item
